...
class Orders extends CI_Controller {
    ...
    function update_no_resi($id)
    {
        $data = $this->order->order_data($id);
        $delivery_data = json_decode($data->delivery_data);
        $delivery_data->no_resi = $this->input->post('no_resi');
        $this->order->set_resi(json_encode($delivery_data), $id);
        $this->session->set_flashdata('order_flash', 'Status input resi');
