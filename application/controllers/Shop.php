...
class Shop extends CI_Controller
{
    public function __construct()
    {
        ...
        $params = array('server_key' => 'SB-Mid-server-I_YrqoaBn6J9RbEkxK7Ee44s', 'production' => false);
        ...
        switch ($action) {
                    ...
                    if ($this->customer->is_coupon_exist($coupon)) {
                        if ($this->customer->is_coupon_active($coupon)) {
                            if ($this->customer->is_coupon_expired($coupon)) {
                                ...
                                $discount = $credit;
                                $disc = '<span class="badge badge-success">' . $coupon . format_rupiah($credit);
                            }
                        } else {
                            $discount = 0;
            ...
            case 'order':
                ...
                $delivery_data = array(
                    ...
                    'note' => $note,
                    'logistic' => $this->input->post('kurir'),
                    'cost_delivery' => $this->input->post('price_delivery'),
                    'estimasi' => $this->input->post('estimasi_delivery'),
                    'no_resi' => null
                );
    ...
    public function cart_api()
    {
        ...
        switch ($action) {
            ...
            case 'remove_item':
                ...
                $data['total']['subtotal'] =  format_rupiah($total_price);
                $data['total']['ongkir'] = ($ongkir > 0) ?  format_rupiah($ongkir) : 'Gratis';
                $data['total']['total'] =  format_rupiah($total_price + $ongkir);
    ...
    function get_ongkir_api()
    {
        $post = $this->input->post();
        $result = json_decode(api('POST', 'cost', $post));
        if ($result->rajaongkir->status->code == 200 && $result->rajaongkir->status->description == "OK") {
            echo json_encode([
                'valid' => true,
                'body' => [
	...
	public function token()
    {
		...
		$transaction_details = array(
		  'order_id' => rand(),
		  'gross_amount' => intval($total_price) // hasil no decimal allowed for creditcard
		);
